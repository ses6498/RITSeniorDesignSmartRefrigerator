	...
	public List<InventoryItem> getInventory()
	{
		try
		{
			...
			for (int i = 0; i < jArray.length(); i++) 
			{
				...
				String dateString = jsonData.getString("expirationDate");
				String parsedString = dateString.substring(0, dateString.indexOf(" "));
				expiration = new Date(parsedString);
				purchased = new Date(jsonData.getString("purchaseDate"));
				
				inventory.add(new InventoryItem(itemName, itemUPC, expiration, purchased));
			}
		}
		catch (JSONException j)
		{
	...
	public List<InventoryItem> getItem(long UPC)
	{
		...
		try
		{
			...
			for (int i = 0; i < jArray.length(); i++) 
			{
				...
				itemUPC = jsonData.getLong("upc");
				itemName = jsonData.getString("description");
				expiration = new Date(jsonData.getString("expirationDate"));
				purchased = new Date(jsonData.getString("purchaseDate"));
