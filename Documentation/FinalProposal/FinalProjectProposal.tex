...
\end{center}
\end{figure}
The majority of software developed will run on the Beagleboard, which will be the main processing platform for the Smart Refrigerator system. The Beagleboard will run the Angstrom operating system, a lightweight embedded Linux. A full Linux environment will be very conducive for software development and will greatly simplify connection with peripherals; the keypad or keyboard, as well as the UPC scanner, will be able to simply ``plug-and-play". The temperature and humidity sensor will require more effort, particularly to ensure the input and output voltage levels are appropriate as to control the sensor properly and not damage the Beagleboard. The RHT03 humidity and temperature sensor used requires a 3.3V-6V power supply and will output voltages as high as the supply voltage. The Beagleboard's general purpose input and output (GPIO) pins supply and recieve voltages up to 1.8V only; however, the pins also incldue a built-in level shifter. This constraint does not imply a need for extra circuitry; though care must be taken to configure the level shifters properly to avoid overvolting the GPIO inputs.
\newline \indent \newline
A more detailed explaination of the Beagleboard subsystem is shown in Figure \ref{fig:basecode}. The figure shows all external connections to the beaglboard as well as an internal seperation into sub-components. The base station code will be inspired by the Model, View, Controller paradigm. A distinct module of code will create the user interface displayed on the Beagleboard's touchpad and relay user interface to a seperate controller module. The controller module will coordinate the various input events generate by the system. Input from the UPC scanner and keyboard will pass through this module and then be transferred to the view. The controller is a necessary middleman in  this process so scanner UPC codes and user inputs can be shared with the model as well. The controller will also be responsible for handling input from the temperature and humidty sensor,  through a GPIO driver implemented. The controller must also interact with the databse structures and handle event from the network interface as well. The model itself will be distributed amoong the remaining modules; the content it needs will be contained within the databases, and the principle modeling task will occur with the expiration date and shopping list prediction algorithms. The two prediction algorithms will be seperated as a unique subsystem, since the prediction task is a unique portion of the system and also since it will belong to a different task group the the overall base station software application.
\begin{figure}[h!]
\vspace{0.5cm}
\begin{center}
...
\end{center}
\end{figure}
The base station application will be developed in C++, using the Qt user interface framework. Java was also given consideration as the primary language, since this could potentially increase consistancy with the mobile application. However, C++ appeared to be a much more common language accross the existing drivers and code examples provided for the Beagleboard. Effort will still be made to maintain consistancy between the two user interaces. To facilitate ease of use and a fluid user experiance between the two applications the layout should be preserved exactly, and aesthetic differances should be minimized. Some initial layouts for the graphical user interface have been designed and are shown in Figures \ref{mock1}, \ref{mock2}, \ref{mock3}, and \ref{mock4}. When designing the interface layouts the constraints of a touch screen interface was considered; all buttons and tabs are intentionally large and easy to click. The Product Entry tab will be the default, and will provide feedback to the user while scanning items. The ``Check In" and ``Check Out" buttons will function as radio buttons to indicate whether the next scanned item will be interpreted as a new purchase or an item being removed from the current inventory. The shopping list tab will provide a straight-forward view of past shopping lists, organized by ascending creation dates. The ``Suggested List" button will produce a new recommended shopping list. The current inventory tab will simply list items currently checked into the refrigerator and provide a reset function to clear the current inventory. As shown in Figure \ref{mock4}, expiration warnings will be presented as pop-up windows. To prevent these warnings from becoming an annoyance to the user the system will attempt to group together multiple alerts.
\begin{figure}[h!]
\vspace{0.5cm}
\begin{center}
...
\end{figure}
\quad \newline
The mobile interface will be divided into similar tabs which potentially will limit the data usage requirements. The mobile application will be updated on a ``need to know" basis only. If additional items have been added to the inventory or new shopping lists have been created on the base station the mobile application will not be notified until the user has opened the application and navigated to the appropriate tab. It is anticipated that the largest data exchanges will need to occur for the shopping lists, since they will require information from multiple items and additional top-level information about the lists. The mobile application will further improve data efficiency by not loading actual shopping list items until a list has been selected. Navigation to the shopping list tab will query for the top-level information about new or updated shopping lists but not delve down into the lists themselves to retrieve item information. Only once a list has been selected will complete updated information about its items be retrieved. This implementation will add additional latency while using the application versus a large of all updated data upon launching the application or a background fetch of information while running the application. However, given the high costs of mobile data plans and  the lack of widespread WiFi at most grocery stores, limiting the amount of data passed across the interface is an important consideration for usability.
\newline \quad \newline
The mobile application should also be tolerant of interrupted connections, since telecommunication networks often have spotty coverage. To mitigate the impact of dropped connections updates should be transferred from the Beagleboard to the mobile application in a sequence of short communications; a large number of small messages will be more robust than a small number of large messages. This structure will also facilitate the ``need to know" distribution of information as well. The processes for handling updates to the current inventory and updates to shopping lists are conceptually similar and are illustrated with a common state machine in Figure \ref{fig:state}. Upon launching the application the update system will enter a waiting state until a tab is selected or some other actions is performed which requires a check for updates, such as expanding a shopping list to view the items within it. The mobile application will  then request the number of relevant updates from the base station software. The state machine will pend until the number of updates is returned and will then enter a state machine evaluates whether there are updates remaining. The system will alternate between pending to receive updates and checking whether additional updates are available. The system will return the waiting state when the ``need to know" information has been retrieved.
\begin{figure}[h!]
\vspace{0.5cm}
\begin{center}
...
\end{center}
\end{figure}
The various databases will be implemented as SQL databases on the Beagleboard. SQL databases will certainly be reliable, and testing will be reduced only to ensure correct interaction with the databases. Figure \ref{fig:databases} shows the division of the multiple databases stored on the Beagleboard. The most intuitive database is the Item Database which will store the total set of UPC codes and corresponding text description. A quantity field will provide not only the quantitiy information but also implicitly provide a check whether an item is in the current inventory or not. The item database will also store the most recent purchase data of an item. However, just the last purchase will not be sufficient for the shopping list prediction algorithm and a separate item history table will be created. The history table will be indexed by UPC, where each element is a list of purchase dates for a particular item. To store shopping lists a shopping list database will be created where each shopping list is given an identification number and name. The shopping lists will also include a flag to indicate whether this shopping list was manual generate or generated by the shopping list creation algorithm; the user may find this information useful in evaluating shopping habits. To coordinate between the list of items and shopping lists themselves and intermediate linking database will store the actual items included on a shopping list. This database will be indexed by the shopping list idea and each element will contain a list of item UPC codes.
\begin{figure}[h!]
\begin{center}
...
\end{figure}
\begin{description}
\item[Extensibility] -- The Smart Refrigerator system presented is only a first step toward tackling food waste, integration of the concepts in this prototype into a larger context could provide enormous utility. Possibly the expiration date and purchasing habit predictions systems could be improved and applied in commercial domains; small restaurants as well as national chains could benefit from more accurate shelf life predictions. The shopping list suggestion algorithm may also be improved by aggregating data across multiple users.
\item[Manufacturability] -- The Smart Refrigerator system will be mainly a software system and will therefore be easy to produce, the same code package could easily be mass-produced. However, the Beagleboard is a very general processing platform and if this prototype system were manufactured commercially a more tailored and application specific platform may be desirable. The system does not have a true need for a  complete Linux environment, though it is ideal for rapid development of a prototype.
\item[Reliability] - The components developed and test externally, such as the Angstrom operating system, will likely be very robust in comparison with the modules developed specifically for this prototype. The user interfaces and prediction algorithms will be tested extensively, though in both cases it is difficult to remove the possibility of error. The mobile interface may not encounter interrupt connections in testing and the shopping list recommendation can not be exposed to all shopping habits in testing and may perform poorly in some scenarios. Fortunately, the system not incur any damages from a failure and will likely not significantly endanger the user in event of a faliure.
\item[Background] -- Experience with Linux based operating systems in the computer science sequence has been helpful, as well as the introduction to user interface development. The project does not contain significant hardware design, though skills learned in Introduction to Digital Systems may be useful in interfacing with the temperature and humidity sensor.
\item[Multidisciplinary Aspects] -- This prototype system requires coordination between hardware and software and requires a mixture of Computer Engineering and Software Engineering skills. However, multidisciplinary projects often carries a mechanical connotation and this system does not require integration of an mechanical components.
\end{description}
...
\hline
\hline
\multicolumn{2}{|c|}{Setup:}&\multicolumn{6}{|p{12cm}|}{User interface subsystem should be entirely integrated with prediction subsystems and SQL databases. System should begin without shopping lists or inventory. System date should be made mutable to facilitate quick simulation of expiration.} \\
\hline
...
Testing of the expiration prediction and shopping list prediction subsystems will be difficult if the system's date can not be adjusted artifically; testing should occur over a few minutes not a series of days. For expiration date testing the system's date should be easy to change artifically, so products appear to expire very quickly. The intelligence of the system can then be tested by providing feedback that test products expired more or less quickly than expected and evaluating the updated predictions. Simplying accelerating the rate with which the system changes date this system can be tested without adding specific test products with low shelf lives and without changing the algorithms to update more frequently. A test sets are listed for this subsystem below.
\begin{itemize}
\item Enter product code and verify that expiration date system is initialized with recommended ``rule of thumb" value.
\item Validate that after providing feedback indicating a product expired before estimate the estimate is decreased. Also validate the opposite case: after providing feedback a product had not yet expired on estimate date the estimate should be increased.
\item Enter a product code and advance system time until product is nearly expired, verify that prediction subsystem has indicated to controller that product is nearing expiration.
\item Rescan product code after regular intervals, indiciative of uni-modal shopping habits. Verify that the system recommends product should be purchases again on this mode. Date may be artifically advanced to faciliatate quick testing.
\item Add outlier shopping habits and validate that system continues to recommend purchasing product again on the mode.
\item Enter various products with different purchasing habits. Verify that system recommends purchasing products with the highest probabilities.
\item Rescan product code after regular intervals, then add significant variation. Observe that system attempts to track variation in habits.
\item Rescan product code after varying intervals, indiciative of bi-modal shopping habits. Verify that system recommends purchasing product on both modes.
\end{itemize}
...
\section{Risks}
The risks unresolved in the prototype system are best grouped by subsystem. Some unresolved risks are simply due to unknown availability of parts, others require more explorations, and finally others are more fundamental risk.
\begin{description}
\item[Availability of LCD Display] -- The ULCD7 lite is the prefered choice for user interface to accompany the Beagleboard. The 7-inch resistive touchscreen is designed to work with the Beagleboard and has drivers built into the Angstrom Linux distribution used. However, the touchscreen does not seem to be stocked by online suppliers and our request through the Arm Developer Day proposal process is still pending. Since the Beagleboard provides a DVI-D and S-video output this risk could be mitigated if the prefered touch screen is not available. The availability of these alternate interfaces also facilitates development while waiting for the desired part. However, since the base station display is a critical system component if this risk is not resolved shortly one of the less desirable alternatives will have to be selected.
\item[Mobile Application Data Exchange] -- Exchanging inventory and shopping lists data with the mobile application presents two risk areas: the amount of data plan usage required and interrupted network connections. These risks are partially mitigated by the design, which exchanges data in small increments on a ``need to know" basis only, however the degree to which these risks are truely problematic is not know. For development the smart phone can be connected with WiFi instead of a costly data connection however, the data requirements should be tested early on in development to ensure this risk does not become a latent problem. The interrupt connection is considered in the design, but the effects of the design considerations made will be difficult to measure since it may be difficult to repeatedly interrupt the data connection.
\item[Practicality of Prediction Algorithms] -- The shopping list prediction algorithm outlined appears optimal for the test data considered. However, the analysis performed did not consider a large sample of shoppers and the conclusions drawn may not be appropriate for a larger population. Also, the prediction algorithm outlined will not perform will with only a few samples and it is unclear how unacceptable this latency will appear to end users. The strategy presented is also quite heavy handed and may be over the top for this prototype system. The system will not serve as an effective shopping aid with an accurate prediction system, but a prototype may be acceptable with a much simpler system which will require significantly less effort.
\item[Efficiency of Database Access] -- Efficient practices for database access were not known or considered when partitioning information into seperate databases and developing the schema for the databases. For example, using the same database to store all possible items and the current inventory may be an inefficient choice. This strategy requires looking over all elements and extracting only those with non-zero quantities. If the system was extended to large  commercial applications or if the Beagleboard was replaced with a more cost-efficient and less powerful alternative this organization may become a signiicant risk.
\end{description}
\pagebreak
\section{Milestones}
\begin{table}[h!]
